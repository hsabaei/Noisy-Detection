...
def train_model(model, train_loader, test_loader, num_epochs, k, device):
    ...
    # per-sample sliding windows (store only losses)
    distance_queues: dict[int, deque] = {}  # idx -> deque[ float ]

    loss_history, test_loss_history = [], []
    lid = LIDEstimators(device=device)

    # cumulative Bayes-GIE sums per sample  (PERSIST across epochs)
    cum_num_bgie: dict[int, float] = defaultdict(float)
    cum_den_bgie: dict[int, float] = defaultdict(float)

    # CKL gate run-length MUST persist across epochs
    runlen_global: dict[int, int] = defaultdict(int)

    G_global = deque(maxlen=k)  # global training-loss reference (length k)
    ...
    for epoch in range(num_epochs):
        ...
        # CKL tracker for this epoch (ref & threshold reset), but reuse run-lengths
        tracker = EpochCklTracker(thr_mode="mean", min_run=5)
        tracker.runlen = runlen_global  # << keep gates persistent over epochs
        ...
        for batch_idx, (inputs, labels, indices, is_noisy) in enumerate(train_loader):
            ...
            # collect phi windows for this batch (None if not enough history)
            phi_list = get_phi_for_indices(distance_queues, indices_np, k_required=k)

            # Bayes-GIE (W,d) for this batch where possible
            W_list, d_list = [], []
            have_bg = []
            for i, idx in enumerate(indices_np):
                if phi_list[i] is None:
                    W_list.append(np.nan); d_list.append(np.nan); have_bg.append(False)
                    continue

                phi = phi_list[i]                  # length == k (because the deque is maxlen=k)
                if len(G_global) < k:              # warm-up: no global reference yet
                    W_list.append(np.nan); d_list.append(np.nan); have_bg.append(False)
                    continue
                G_tr = np.asarray(list(G_global), dtype=float)  # length == k; same timestamps as φ


                NG0, DG0 = cum_num_bgie[int(idx)], cum_den_bgie[int(idx)]
                bayes_val, W_i, Num_inc, Den_inc = lid.compute_Bayes_GIE(phi, G_tr, NG0, DG0)

                # BUGFIX: accumulate (do NOT overwrite)
            ...
            # CKL reference (union reference so far this epoch)
            ref = tracker.current_ref()
            if ref is None:
                # bootstrap from current batch BGIE if available
                valid = np.isfinite(W_b) & np.isfinite(d_b) & (W_b > 0) & (d_b > 0)
                if not np.any(valid):
                    # no BGIE yet: fall back to standard CE
            ...
            # CKL for samples with BGIE; NaN otherwise
            ...
            # threshold BEFORE updating with current batch
            thr_now = tracker.current_thr()

            # α from CKL where available, else 1.0; apply persistent gate
            ...
            if np.any(np.isfinite(ckl_vals)):
                a_calc = ckl_to_alpha(ckl_vals[np.isfinite(ckl_vals)], thr_now, kappa=2.0, alpha_floor=0.05)
            ...
            # update reference & threshold with current batch BGIE
            valid = np.isfinite(W_b) & np.isfinite(d_b) & (W_b > 0) & (d_b > 0)
            if np.any(valid):
