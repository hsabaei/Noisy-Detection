...
def train_model(model, train_loader, test_loader, num_epochs, k, device):
    ...
    # Per-sample φ queues (now φ = logit at provided class)
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, indices, is_noisy in train_loader:
            ...
            # ---------- φ: LOGIT value at the PROVIDED (input) class ----------
            with torch.no_grad():
                ar = torch.arange(labels.size(0), device=labels.device)
                phi_vals = logits[ar, labels]          # shape [B], unbounded logits
        ...
        if epoch >= k - 1:
            for idx, dq in phi_queues.items():
                ...
                phi_seq, ep_seq = zip(*dq)
                phi  = np.array(phi_seq, dtype=float)                 # φ = logit(label)
    ...
    print("Saved pairs for cohorts: cat / dog / noisy (φ = logit of INPUT class).")
