...
def build_union_reference(dog, noisy):
    ...
    for t in range(max_T):
        ...
        refs[t] = ( math.exp(huber_mean(logWs)),
                    ...
                    len(logWs) )
    return refs  # epoch -> (W_ref, d_ref, n_used)

# ---------- Per-epoch CKL + mean(log) threshold + consecutive gate + enrichment
def per_epoch_metrics_with_gate(dog, noisy, refs, min_run=3):
    ...
    for t in range(max_T):
        ...
        epoch_rows = []  # (sid, cohort, w, d, logd, score, log_score)
        for sid, seq in dog.items():
            if len(seq) > t:
                w, d, logd = seq[t]
                score = ckl_sym(w, d, W_ref, d_ref) if USE_SYMMETRIC else ckl_finite_boundary(w, d, W_ref, d_ref)
                log_score = math.log(max(score, EPS)) if np.isfinite(score) else -np.inf
                epoch_rows.append((sid, "dog_clean", w, d, logd, score, log_score))
        for sid, seq in noisy.items():
            if len(seq) > t:
                w, d, logd = seq[t]
                score = ckl_sym(w, d, W_ref, d_ref) if USE_SYMMETRIC else ckl_finite_boundary(w, d, W_ref, d_ref)
                log_score = math.log(max(score, EPS)) if np.isfinite(score) else -np.inf
                epoch_rows.append((sid, "dog_noisy", w, d, logd, score, log_score))
        if not epoch_rows:
            continue

        # 2) threshold = mean of log scores at epoch
        log_scores = [r[6] for r in epoch_rows if np.isfinite(r[6])]
        thr_log = float(np.mean(log_scores) ) if log_scores else np.nan
        thr_linear = float(math.exp(thr_log)) if np.isfinite(thr_log) else np.nan  # for reference

        # 3) raw & gated flags in LOG space; accumulate counts
        n_clean_avail = n_noisy_avail = 0
        hit_clean_raw = hit_noisy_raw = 0
        hit_clean_gate = hit_noisy_gate = 0

        for sid, cohort, w, d, logd, score, log_score in epoch_rows:
            raw_flag = np.isfinite(log_score) and np.isfinite(thr_log) and (log_score > thr_log)
            ...
            rows.append({
                ...
                "CKL": score, "log_CKL": log_score,
                "threshold_log": thr_log, "threshold_linear": thr_linear,
...
print("\n[Head] per-epoch counts with gate & enrichment (mean log threshold):")
