...
#    scheduler = CosineAnnealingLR(optimizer, T_max=NUM_EPOCHS, eta_min=1e-3)
    ...
    for epoch in range(num_epochs):
        ...
        if USE_CKL:
            tracker = ClassCklTracker(num_classes=10, thr_mode="median", min_run=3)
        ...
        if USE_CKL and (epoch + 1) % 10 == 0:
            ...
            with torch.no_grad():
                for inputs, labels, indices, is_noisy in train_loader:
                    ...
                    # Recompute CKL/gating for this batch (same logic as training)
                    logits = model(inputs)
                    ...
                    # Use same tracker (already updated during training)
                    ckl_vals = np.full(len(indices_np), np.nan, dtype=np.float64)
                    for i in range(len(indices_np)):
