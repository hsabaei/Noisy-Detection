...
def train_model(model, train_loader, test_loader, num_epochs, k, device):
    ...
    # Per-sample φ queues (now φ = P(model assigns to PROVIDED label))
    phi_queues = {}  # idx -> deque of (phi, epoch)

    loss_history, test_loss_history = [], []
    lid = LIDEstimators(device=device)

    # Bayes accumulators (cumulative)
    num_bfie, den_bfie = {}, {}
    num_bgie, den_bgie = {}, {}
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, indices, is_noisy in train_loader:
            ...
            # forward
            logits = model(inputs)
            batch_loss = global_criterion(logits, labels)
            ...
            running_loss_sum += float(batch_loss.item()) * labels.size(0)
            running_count    += int(labels.numel())

            # ---------- φ: softmax probability of the INPUT (provided) class ----------
            with torch.no_grad():
                probs = F.softmax(logits, dim=1)  # [B, C]
                ar = torch.arange(labels.size(0), device=labels.device)
                phi_vals = probs[ar, labels]      # [B], in [0,1]
                phi_vals = phi_vals.detach().cpu()

            # enqueue per-sample φ
            for i in range(len(indices)):
                idx = int(indices[i])
                if idx not in phi_queues:
                    phi_queues[idx] = deque(maxlen=k)
                phi_queues[idx].append((float(phi_vals[i]), epoch))

        # record epoch train loss
        loss_history.append(running_loss_sum / max(running_count, 1))

        # record epoch test loss
        model.eval()
        t_sum, t_cnt = 0.0, 0
        with torch.no_grad():
            for x, y in test_loader:
                ...
                logits = model(x)
                t_sum += float(global_criterion(logits, y).item()) * y.size(0)
                t_cnt += int(y.numel())
        test_loss_history.append(t_sum / max(t_cnt, 1))

        # ---- compute FIE/GIE once window is filled ----
        if epoch >= k - 1:
            for idx, dq in phi_queues.items():
                if len(dq) < k:
                    continue

                conf_seq, ep_seq = zip(*dq)
                phi  = np.array(conf_seq, dtype=float)               # φ = P(label)
                G_tr = np.array([loss_history[e] for e in ep_seq], dtype=float)

                # FIE* on φ (previous vs next)
                fie_star, w_fie = lid.compute_GIE_LID(phi[:-1], phi[1:])

                NF0 = num_bfie.get(idx, 0.0)
                DF0 = den_bfie.get(idx, 0.0)
                bayes_fie, wb_fie, Num_fie, Den_fie = lid.compute_Bayes_GIE(phi[:-1], phi[1:], NF0, DF0)
                num_bfie[idx] = Num_fie
                den_bfie[idx] = Den_fie

                # GIE on (φ, G_train)
                gie_tr, w_tr = lid.compute_GIE_LID(phi, G_tr)

                NG0 = num_bgie.get(idx, 0.0)
                DG0 = den_bgie.get(idx, 0.0)
                ...
                def _push(dct, _idx, w, val):
                    w = float(max(w, EPS))
                    v = float(val) if np.isfinite(val) else 0.0
                    dct.setdefault(_idx, []).append((w, v))
    ...
    print("Saved pairs for cohorts: cat / dog / noisy (φ = softmax prob of INPUT class).")
