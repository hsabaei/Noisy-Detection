...
def train_model(model, train_loader, test_loader, num_epochs, k, device):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, indices, is_noisy in train_loader:
            ...
            one_hot = F.one_hot(labels, num_classes=probs.size(1)).float()       # [B, C]
            phi_vals = torch.linalg.norm(probs - one_hot, ord=2, dim=1)   
            ...
            for i in range(len(indices)):
                ...
                distance_queues[idx].append((float(phi_vals[i].detach().cpu()), epoch))
