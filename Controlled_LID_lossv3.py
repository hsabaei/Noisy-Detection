...
def train_model(model, train_loader, test_loader, num_epochs, k, device):
    ...
    distance_queues = {}   # idx -> deque of (phi_t, epoch)
    ...
    # --- NEW: class prototypes from previous epoch (None until we finish epoch 0)
    num_classes = 10
    proto_prev = [None] * num_classes

    # temp accumulators for this epoch’s prototypes
    def reset_proto_acc():
        return [np.zeros(64, dtype=np.float32) for _ in range(num_classes)], [0]*num_classes
    proto_sum, proto_cnt = reset_proto_acc()

    # outputs split by cohort (unchanged)
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels, indices, is_noisy in train_loader:
            ...
            # penultimate features for φ and prototypes
            feats = model.penultimate.detach().cpu().numpy()   # [B, 64]
            lbls  = labels.detach().cpu().numpy()
            idxs  = indices.detach().cpu().numpy()

            # --- build φ_t = log(EPS + ||f_t - proto_prev[label]||_2)
            #     (only if we already have previous-epoch prototypes)
            have_prev = proto_prev[0] is not None
            if have_prev:
                for i, idx in enumerate(idxs):
                    c = int(lbls[i])
                    mu_prev = proto_prev[c]
                    if mu_prev is not None:
                        dist = float(np.linalg.norm(feats[i] - mu_prev))
                        phi_t = float(np.log(EPS + dist))
                        dq = distance_queues.get(int(idx))
                        if dq is None:
                            dq = deque(maxlen=k)
                            distance_queues[int(idx)] = dq
                        dq.append((phi_t, epoch))

            # accumulate this epoch’s prototype stats
        ...
        # compute global train loss for this epoch (G)
        loss_history.append(running_loss_sum / max(running_count, 1))

        # --- roll prototypes: end of epoch, set proto_prev = means from this epoch
        for c in range(num_classes):
            if proto_cnt[c] > 0:
                proto_prev[c] = (proto_sum[c] / max(proto_cnt[c], 1)).astype(np.float32)
            # if a class had no samples (won’t happen on CIFAR-10 train), leave as-is

        # test loss (monitoring / optional GIE variant)
        model.eval()
        t_sum, t_cnt = 0.0, 0
        with torch.no_grad():
        ...
        if epoch >= k - 1:
            for idx, dq in distance_queues.items():
                if len(dq) < k:
                    continue
                dists, epochs_vec = zip(*dq)
                phi  = np.array(dists, dtype=float)
                G_tr = np.array([loss_history[e] for e in epochs_vec], dtype=float)

                # FIE* on successive φ (your existing estimator)
                fie_star, w_fie = lid.compute_GIE_LID(phi[:-1], phi[1:])
                NF0, DF0 = num_bfie.get(idx, 0.0), den_bfie.get(idx, 0.0)
                bayes_fie, wb_fie, Num_fie, Den_fie = lid.compute_Bayes_GIE(phi[:-1], phi[1:], NF0, DF0)
                num_bfie[idx], den_bfie[idx] = Num_fie, Den_fie

                # GIE train (φ vs global train loss G)
                gie_tr, w_tr = lid.compute_GIE_LID(phi, G_tr)
                NG0, DG0 = num_bgie.get(idx, 0.0), den_bgie.get(idx, 0.0)
                bayes_tr, wb_tr, Num_tr, Den_tr = lid.compute_Bayes_GIE(phi, G_tr, NG0, DG0)
                num_bgie[idx], den_bgie[idx] = Num_tr, Den_tr

                cohort = train_loader.dataset.group_map.get(idx, 'other')
                def _push(dct, idx_, w, val):
                    w = float(max(w, EPS))
                    v = float(val) if np.isfinite(val) else 0.0
                    dct.setdefault(idx_, []).append((w, v))

                if cohort == 'cat':
                    _push(w_fie_cat, idx, w_fie, fie_star);  _push(w_gie_cat, idx, w_tr, gie_tr)
                    _push(w_bfie_cat, idx, wb_fie, bayes_fie); _push(w_bgie_cat, idx, wb_tr, bayes_tr)
                elif cohort == 'dog':
                    _push(w_fie_dog, idx, w_fie, fie_star);  _push(w_gie_dog, idx, w_tr, gie_tr)
                    _push(w_bfie_dog, idx, wb_fie, bayes_fie); _push(w_bgie_dog, idx, wb_tr, bayes_tr)
                elif cohort == 'noisy':
                    _push(w_fie_noisy, idx, w_fie, fie_star);  _push(w_gie_noisy, idx, w_tr, gie_tr)
                    _push(w_bfie_noisy, idx, wb_fie, bayes_fie); _push(w_bgie_noisy, idx, wb_tr, bayes_tr)
    ...
    save_pairs_to_file(w_gie_cat,   "cat_GIE_pairs.json")
    save_pairs_to_file(w_gie_dog,   "dog_GIE_pairs.json")
    save_pairs_to_file(w_gie_noisy, "noisy_GIE_pairs.json")
    save_pairs_to_file(w_bfie_cat,  "cat_BFIE_pairs.json")
    save_pairs_to_file(w_bfie_dog,  "dog_BFIE_pairs.json")
    save_pairs_to_file(w_bfie_noisy,"noisy_BFIE_pairs.json")
    save_pairs_to_file(w_bgie_cat,  "cat_BGIE_pairs.json")
    save_pairs_to_file(w_bgie_dog,  "dog_BGIE_pairs.json")
    save_pairs_to_file(w_bgie_noisy,"noisy_BGIE_pairs.json")
